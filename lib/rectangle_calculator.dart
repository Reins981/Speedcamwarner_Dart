...
class PredictiveModel {
  ...
  final List<math.Point<double>> cameras;
...
Future<SpeedCameraEvent?> predictSpeedCamera({
  ...
  final current = math.Point<double>(latitude, longitude);
...
class RectangleCalculatorThread {
    ...
    if (predictor == null) {
      ...
      unawaited(() async {
        try {
          ...
          final timeOfDay = _formatTimeOfDay(hour);

          final predicted = await predictor!.predict(
            latitude: latitude,
            longitude: longitude,
            timeOfDay: timeOfDay,
          ...
          logger.printLogLine("Predictive lookup failed: $e");
          logger.printLogLine(st.toString());
        }
      }());
    }
  ...
  Future<void> fetchLufopCameras({http.Client? client}) async {
    final url =
        'https://api.lufop.net/api?key=$lufopKey&format=json&q=$latitude,$longitude&m=300&nbr=10000';
