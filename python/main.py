...
class MaxSpeedlayout(FloatLayout):
    ...
    def __init__(self, gps_layout):
        ...
        self.imtrafficcam = UIXImage(source='', nocache=True, pos_hint={"top": 3},
                                     size_hint=(0, 0))
    ...
    def callback_poi(self, instance):
        ...
        if isinstance(gps_producer, GPSThread) and isinstance(calculator,
            ...
            if status == 'OK' and len(data) > 0:
                ...
                self.main_app.ms.update_online_image_layout("UNDEFINED")
                ...
                self.voice_prompt_queue.produce_poi_status(self.cv_voice, "POI_FAILED")
                if status != "OK":
                    self.main_app.ms.update_online_image_layout("INETFAILED")
        else:
            self.voice_prompt_queue.produce_poi_status(self.cv_voice, "POI_FAILED")
...
class Gpslayout(BoxLayout):
    ...
    def callback_ar_human(self, instance):
        self.camera.source = 'images/human.jpg'
        self.camera.color = (1, .9, 0, 2)
        self.camera.texture_update()
    ...
    def update_ar(self, m_type='human'):
        if m_type == 'human':
            Clock.schedule_once(self.callback_ar_human)
...
class MainTApp(App):
    ...
    def callback_day_night(self, instance):
        if self.nightbutton.text == 'Night':
            ...
            self.ml.speedcam_fix_number.color = (.5, 0, .8, 1)
            self.ml.speedcam_fix_number.texture_update()
            self.ml.speedcam_mobil_number.color = (.5, 0, .8, 1)
            self.ml.speedcam_mobil_number.texture_update()
            self.ml.speedcam_trafficlight_number.color = (.5, 0, .8, 1)
            self.ml.speedcam_trafficlight_number.texture_update()
            self.ml.speedcam_distance_number.color = (.5, 0, .8, 1)
            self.ml.speedcam_distance_number.texture_update()
            self.ml.poi_number.color = (.5, 0, .8, 1)
            self.ml.poi_number.texture_update()
            self.s.service_unit.color = (.5, 0, .8, 1)
            self.s.service_unit.texture_update()
            self.root_table.poibutton.color = (.5, 0, .8, 1)
            self.root_table.poibutton.texture_update()
            self.root_table.label_h.color = (.5, 0, .8, 1)
